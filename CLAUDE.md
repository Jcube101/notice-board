...
- **Reactions field:** reactions live as a `reactions` **JSON map** on the `notes` collection, e.g. `{ '👋': 3, '🔥': 1 }` (emoji → count). A `null` or missing `reactions` field must be treated as an **empty map** `{}`, not an error.
